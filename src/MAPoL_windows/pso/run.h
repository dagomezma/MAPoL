...
inline void atualizaMelhor(std::vector<Solucao> *melhores,
		std::vector<double> *Mg, double *fitMg, std::vector<Particula> &enxame, unsigned int it) {
	bool mudar = false;
	unsigned int id;
	for (unsigned int i = 0; i < enxame.size(); ++i) {
...
void run(std::vector<Solucao> *melhores, double w_max, double w_min, double c,
		double s, unsigned int nParticulas, unsigned int nIteracoes, unsigned int tamParticula,
...
//	debug("Fitness = %lf\n", mkl_runpf(*estrutura, enxame[0]));
	for (unsigned int i = 1; i < nParticulas; ++i) {
	...
	// Loop principal
	for (unsigned int it = 1; it <= nIteracoes; ++it) {
