...
public:
	struct Estrutura {
		...
		Estrutura(Tipo tipo, double min, double max, double def, unsigned int qtd,
			unsigned int id) :
				tipo(tipo), min(min), max(max), defaultValue(def), qtd(qtd), id(
						id) {
		}

		static Estrutura newAVR(double min, double max, double def, unsigned int id) {
			return Estrutura(AVR, min, max, def, 0, id);
		}

		static Estrutura newOLTC(double min, double max, double def, unsigned int qtd,
			unsigned int id) {
			return Estrutura(OLTC, min, max, def, qtd, id);
		}

		static Estrutura newShC(double max, double def, unsigned int qtd, unsigned int id) {
	...
	double operator[](unsigned int posicao) {
		return X[posicao];
	}

	/* Setters */

	void inicializar(unsigned int tamanho, std::vector<double> *Mg,
